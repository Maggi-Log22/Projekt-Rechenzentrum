# K-Means auf allen Subjects (1-20)
import wfdb
...
axs[1].plot(list(k_range), silhouettes, marker="o", color="green")
...
plt.title(f"K-Means (k={k_manual}) – PCA 2D")
plt.xlabel("PC1")
plt.ylabel("PC2")
...
# --- Interpretation: Cluster-Mittelwerte in Originalskala ---
cluster_df = pd.DataFrame(clipped, columns=feature_cols)
cluster_df["cluster"] = labels
cluster_means = cluster_df.groupby("cluster").mean().round(3)

# Ausgabe als eigenes Fenster
fig = plt.figure(figsize=(14, 6))
plt.title("Cluster-Mittelwerte (Originalskala, gerundet)")
plt.axis("off")
table = plt.table(
    cellText=cluster_means.values,
...
try:
    ...
    table.scale(1, 1.2)
    table.auto_set_column_width(col=list(range(len(cluster_means.columns))))
except Exception:
    pass
...
interp = cluster_means[["acc_mag_mean", "hr_mean", "eda_mean"]].copy()
...
print("\nInterpretation (Heuristik) pro Cluster:")
